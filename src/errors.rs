...
impl fmt::Display for WhisperRealtimeErrorType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
       match self {
           WhisperRealtimeErrorType::TranscriptionError => {write!(f, "TranscriptionError")}
           WhisperRealtimeErrorType::DownloadError => {write!(f, "DownloadError")}
           WhisperRealtimeErrorType::Unknown => {write!(f, "UnknownError")}
       }
    }
}
